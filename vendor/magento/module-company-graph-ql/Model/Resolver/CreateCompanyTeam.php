...
class CreateCompanyTeam implements ResolverInterface
{
    ...
    public function resolve(Field $field, $context, ResolveInfo $info, array $value = null, array $args = null)
    {
        ...
        try {
            ...
            $newTeam->setHasDataChanges(true);
            $this->teamRepository->create($newTeam, $company->getId());
            $team = $this->teamRepository->get((int)$newTeam->getId());
            $teamStructureId = $this->structure->getStructureByTeamId($team->getId())->getId();
            $this->structure->moveNode($teamStructureId, (int)$preparedData['target_id']);
        } catch (\Exception $e) {
            throw new LocalizedException(
                __('Team could not be created'),
                $e
            );
        }
