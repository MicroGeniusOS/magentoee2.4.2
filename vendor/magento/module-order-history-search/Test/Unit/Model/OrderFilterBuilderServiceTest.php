...
class OrderFilterBuilderServiceTest extends TestCase
{
     ...
     * @return void
     */
    public function testApplyOrderFiltersWithFullParams()
    {
        ...
        $appliedFilterIteration = 0;

        foreach ($filterParams as $value) {
            $filterMock->expects($this->at($appliedFilterIteration))
                ->method('applyFilter')
                ->with($collectionMock, $value)
                ->willReturn($collectionMock);

            $appliedFilterIteration++;
        }
     ...
     * @return void
     */
    public function testApplyOrderFilterWithEmptyParams()
    {
        $collectionMock = $this->collectionMock;
