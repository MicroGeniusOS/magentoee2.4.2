...
class OrderAddressProviderTest extends TestCase
{
    ...
    public function testGetByCustomerId()
    {
        ...
        $this->searchCriteriaBuilderMock->expects($this->at(0))->method('addFilter')->with(
            OrderAddressInterface::ADDRESS_TYPE,
            Address::TYPE_SHIPPING
        )->willReturnSelf();
        $this->searchCriteriaBuilderMock->expects($this->at(1))->method('addFilter')->with(
            OrderAddressInterface::PARENT_ID,
            [],
            'in'
        )->willReturnSelf();
